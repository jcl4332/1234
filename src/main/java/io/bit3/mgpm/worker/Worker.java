...
public class Worker implements Runnable {
    ...
    public void run() {
        ...
        } catch (GitProcessException | WorkerException exception) {
    ...
    private boolean cloneOrReconfigureRepository() throws WorkerException, GitProcessException {
        ...
        if (directory.exists()) {
            ...
            if (new File(directory, ".git").isDirectory()) {
                String actualUrl = git(CONF, LOCAL, GET, "remote.origin.url");
    ...
    private void determineLocalBranchesAndUpstreams() throws GitProcessException {
        ...
        for (String branchName : localBranchNames) {
            ...
            remoteName = remoteNameTry(branchName, remoteName);

            remoteRef = remoteRefTry(branchName, remoteRef);

            rebase = rebaseTry(branchName, rebase);
            ...
            if (remoteRef!=null && remoteRef.startsWith("refs/heads/")) {
    ...
    private String rebaseTry(String branchName, String rebase) {
        try {
            rebase = git(CONF, LOCAL, GET, String.format("branch.%s.rebase", branchName)).toLowerCase();

            if (StringUtils.isBlank(rebase)) {
                // not defined for this branch, use global setting instead
                rebase = git(CONF, GET, "pull.rebase");
            }
        } catch (GitProcessException e) {
    ...
    private String remoteRefTry(String branchName, String remoteRef) {
        try {
            remoteRef = git(CONF, LOCAL, GET, String.format("branch.%s.merge", branchName));
        } catch (GitProcessException e) {
            // exception means, there is no remote configured
        }
        return remoteRef;
    ...
    private String remoteNameTry(String branchName, String remoteName) {
        try {
            remoteName = git(CONF, LOCAL, GET, String.format("branch.%s.remote", branchName));
        } catch (GitProcessException e) {
            // exception means, there is no remote configured
        }
        return remoteName;
